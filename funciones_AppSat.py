...
def read_file_pdf(Archivos, path_raiz_cliente):
    ...
    """
    try:
        conexion1 = r"C:/Users/Administrator/Documents/Aplicacion_SAT/dataframe"
        # conexion2 = "mssql+pyodbc://sa:Foretagbi_01@BDD\\APLICACIONSAT/AppSat?driver=ODBC+Driver+17+for+SQL+Server"
        # conexion2 = "postgresql://foretagbi:foretagbi@localhost/foretagbi"
        conexion2 = "postgresql://postgres:Foretagbi@localhost/postgres"
        ...
        # Formato de Informacion de contribuyente LH
            ...
            exportData_Info_Contribuyente(txt, Archivos, Tipo_pdf, conexion1, conexion2)
        ...
        # Formato de Pagos
            ...
            exportDataPagos(txt, Archivos, Tipo_pdf, conexion1, conexion2)
...
def Clasificador_Archivos(rutas, path_raiz_cliente,  fecha_limite=None):
    ...
    if fecha_limite is None:
        ...
        fecha_limite = fecha_limite - datetime.timedelta(days=1)
        print("Ejecuta desde:", fecha_limite)
    else:
...
def procesar_archivo_last(file_path, path_raiz_cliente, fecha_limite):
    fecha_creacion = datetime.datetime.fromtimestamp(os.path.getctime(file_path))
    if fecha_creacion >= fecha_limite and file_path.endswith(".pdf"):
        Carpeta_ejercicio = fecha_creacion.year
        dic_document = read_file_pdf(file_path, os.path.join(path_raiz_cliente, str(Carpeta_ejercicio)))
        return dic_document
    else:
        return None

def Clasificador_Archivos_last(rutas, path_raiz_cliente, fecha_limite=None):
    if fecha_limite is None:
        fecha_limite = datetime.datetime.now()
    else:
        fecha_limite = datetime.datetime.strptime(fecha_limite, "%Y-%m-%d")
        ...
        with Pool() as pool:
            for root in rutas:
                for path, subdirs, files in os.walk(root):
                    ...
                    results = pool.starmap(procesar_archivo, [(file_path, path_raiz_cliente, fecha_limite) for file_path in file_paths])
                    array_docs.extend([doc for doc in results if doc is not None])
...
def exportDataDeclaracion(pdf_txt, path_pdf, Tipo_pdf, conexion1, conexion2):
    try:
        ...
        print("Error en funcion exportDataDeclaracion: ", e, " ->archivo:", path_pdf)
...
def exportDataPagos(pdf_txt, path_pdf, Tipo_pdf, conexion1, conexion2):
    try:
        ...
        print("Error en funcion exportDataPagos: ", e, " ->archivo:", path_pdf)
